...
void check_models(const RaxmlInstance& instance)
{
  ...
  for (const auto& pinfo: instance.parted_msa->part_list())
  {
    ...
    auto freq_mode = model.param_mode(CORAX_OPT_PARAM_FREQUENCIES);
...
void predict_msa_difficulty(RaxmlInstance& instance)
{
  ...
  if((!diff_pred) || !opts.use_pythia || instance.opts.modified_version)
    return;
...
void autotune_start_trees(RaxmlInstance& instance)
{
  ...
  else
  {
    if (opts.constraint_tree_file.empty() || !opts.use_old_constraint)
    {
      ...
      opts.start_trees[StartingTree::random] = difficulty >= 0.8 ? 0 :
          diff_pred->num_start_trees(difficulty, 3.5, 0.5, 0.25);
      opts.start_trees[StartingTree::parsimony] = difficulty >= 0.8 ? (int) (1.5 * pars_trees) : pars_trees;
...
//    printf("\n\n");
  ...
  if (opts.command == Command::bsmsa || opts.write_bs_msa)
  {
    if (!opts.bootstrap_msa_file(1).empty())
    {
      ...
      auto max_bs_trees = opts.write_bs_msa ? checkp.bs_trees.size() : opts.num_bootstraps;
...
void init_parallel_buffers(const RaxmlInstance& instance)
{
  ...
  if (ParallelContext::num_ranks() > 1)
  {
    ...
    {
      // add 1 extra tree for load imbalance
      auto max_trees_per_worker = (opts.bootstop_interval / ParallelContext::num_groups() + 1) + 1;
      auto max_trees_per_rank = max_trees_per_worker * ParallelContext::num_local_groups();
