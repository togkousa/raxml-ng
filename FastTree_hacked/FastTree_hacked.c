...
void LogTree(char *format, 
            int round, /*OPTIONAL WRITE*/
            FILE *fp, 
            NJ_t *NJ, 
            char **names, 
            uniquify_t *unique, 
...
void LogTreeIntermediate(FILE *fpLog, 
                          NJ_t *NJ, 
                          char **names, 
                          uniquify_t *unique, 
                          bool bQuote,
...
char *remove_extension(char* myStr, char extSep, char pathSep);
char* concatenate_strings(const char *s1, const char *s2);
void store_intermediate_tree(const char *filepath, 
                            NJ_t *NJ, 
                            char **names, 
                            uniquify_t *unique, 
                            bool bQuote,
...
int main(int argc, char **argv) {
  ...
  for(iAln = 0; iAln < nAlign; iAln++) {
	...
	if (spr > 0 || nniToDo > 0 || MLnniToDo > 0)
      ...
      LogTreeIntermediate(fpLog, NJ, aln->names, unique, bQuote, false);
	...
	for (i=0; i < nniToDo; i++) {
	  double maxDelta;
	  if (!bConverged) {
	    int nChange = NNI(/*IN/OUT*/NJ, i, nniToDo, /*use ml*/false, /*IN/OUT*/nni_stats, /*OUT*/&maxDelta, fpLog, aln, unique, bQuote);
	  ...
	  if (sprRemaining > 0 && (nniToDo/(spr+1) > 0 && ((i+1) % (nniToDo/(spr+1))) == 0)) {
	    SPR(/*IN/OUT*/NJ, maxSPRLength, spr-sprRemaining, spr, fpLog, aln, unique, bQuote);
  ...
  if (verbose>1) {
	...
	for (iMLnni = 0; iMLnni < MLnniToDo; iMLnni++) {
	  int changes = NNI(/*IN/OUT*/NJ, iMLnni, MLnniToDo, /*use ml*/true, /*IN/OUT*/nni_stats, /*OUT*/&maxDelta, fpLog, aln, unique, bQuote);
...
{   
    ...
    if (fp != NULL)
    {
        PrintNJ(fp, NJ, names, unique, false, bQuote);
        fflush(fp);
        fclose(fp);
    }
}
                          ...
                          bool append)
{
  if(fpLog != NULL) {
    ...
    filename[r] = '\0';
    char *filename_no_extension = remove_extension(filename, '.', '/');
    char *spt_trees_file = concatenate_strings(filename_no_extension, ".chkpTrees");
    
    store_intermediate_tree(spt_trees_file, NJ, names, unique, bQuote, append);
  }

}

void LogTree(char *format,
 int i, /*OPTIONAL WRITE*/
 FILE *fpLog, 
 NJ_t *NJ, 
 char **names, 
 uniquify_t *unique, 
...
NJ_t *InitNJ(char **sequences, int nSeq, int nPos,
  ...
  for (iNode = 0; iNode < NJ->maxnodes; iNode++) NJ->branchlength[iNode] = 0.1;
  ...
  *dMaxDelta = 0.0;
  ...
  while((node = TraversePostorder(node, NJ, /*IN/OUT*/traversal, &bUp)) >= 0) {
    ...
    if (useML) {
      ...
      */
     // HERE I STORE THE NEW TOPOLOGY I GUESS (2)
      if (choice == ADvsBC) {
	...
	newlength[LEN_C] = tmp;
        ...
        // HERE I STORE THE NEW TOPOLOGY I GUESS (3)
	/* swap B and C */
    ...
    if (choice == ABvsCD) {
      ...
      // HERE I STORE THE NEW TOPOLOGY I GUESS (4) -- This is the place I suppose
      LogTreeIntermediate(fpLog, NJ, aln->names, unique, bQuote, true);
...
void SPR(/*IN/OUT*/NJ_t *NJ, 
  ...
  */
  ...
  for (i = 0; i < nodeListLen; i++) {
    ...
    if (bChanged) {
      // HERE I STORE THE NEW TOPOLOGY I GUESS (6)
